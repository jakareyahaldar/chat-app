...
export default function EmojiBox(Props){
  ...
  const { setEmoji, isOpen, tailwind, x, y, setReactBoxData } = Props
  ...
  function handleClose(){
    setReactBoxData(p=>({...p,show:false}))
  }
  ...
  return(
    <div ref={BoxEl} className={`${tailwind} md:w-[300px] w-fit max-h-[300px] overflow-y-scroll bg-amber-100 rounded-xl grid grid-cols-6 gap-3 p-3 pt-8 m-auto`}>
      
      <i onClick={handleClose} className="absolute top-1.5 right-1.5 text-black fa-solid fa-xmark"></i>
